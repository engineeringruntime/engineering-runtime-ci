...
Bootstrap also seeds `runtime-home/capabilities/` from the binary, and the
smoke/demo workflows execute those **by name**
(`runtime capability validate github/github-repo-health`) — no
`RUNTIME_CAPABILITIES_DIR`, because Home is the default. That variable is
reserved for the **company capability store**
([`capabilities-from-registry.yaml`](.github/workflows/capabilities-from-registry.yaml)
and the scaffold/ship demos). Never point it into this working tree —
every `runtime` invocation re-runs bootstrap and would seed the binary's
full capability tree there.

This repo checks in **no** capability Markdown. What CI runs is what the
binary ships; the source of truth is
[`engineering-runtime/capabilities/`](https://github.com/kishore-gutta/engineering-runtime/tree/main/capabilities).
...
|---|---|---|
| [`runtime-ci.yaml`](.github/workflows/runtime-ci.yaml) | push/PR/dispatch | Install → bootstrap home → `config validate` → validate every seeded capability → `files/notes-roundtrip` → `auth` → `github user get` / `org list` / `notification list` → audit |
...
| [`java-service-scaffold-and-ship.yaml`](.github/workflows/java-service-scaffold-and-ship.yaml) | dispatch | **Replay demo:** clone **latest** Capability Registry (`engineering-runtime-capabilities@main`) → `RUNTIME_CAPABILITIES_DIR` → create fresh repo → execute `github/java-service-scaffold-and-ship` by name (runtime only, no AI) |
...
[`java-service-scaffold-and-ship.yaml`](.github/workflows/java-service-scaffold-and-ship.yaml)
demo (creates a fresh repo each run).
...
runtime-home/               # generated by bootstrap (gitignored) — config, policy, specs, commands
  capabilities/              # seeded from the binary; workflows execute these by name
```
