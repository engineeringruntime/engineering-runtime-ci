...
| `RUNTIME_CAPABILITIES_DIR` | Point at the **company capability store** after bootstrap (GitHub clone, registry unpack dir, shared path — see §F). Never point it into this repo's working tree — every `runtime` invocation re-runs bootstrap and would seed the binary's full capability tree there. Leave it **unset** for smoke/demo workflows, which resolve names against `<Home>/capabilities` |
...
The capabilities the workflows run are the ones **bootstrap seeds from the
binary** into `runtime-home/capabilities/`, executed **by name**
(`github/github-repo-health`, not a path). They are both **regression
fixtures** and **demo scripts**: a human or customer should be able to open
Actions → run workflow → read JSON/audit output and understand what the
runtime did, without cloning `engineering-runtime` or configuring a laptop.

This repo **vendors no capability Markdown of its own.** It used to keep a
copy under an `examples/` directory; that copy drifted from
`../engineering-runtime/capabilities/` and CI proved the drifted text
instead of what customers install. Whatever the workflows execute must be
what the binary ships — so if a demo needs a capability that does not exist
yet, add it to `../engineering-runtime/capabilities/` (or the company
store, §F), never here.
...
leaning on the bootstrap-seeded Home — the seeded copies stay for
smoke/demos; the company store path is what customers will actually run.

**Two sources, one resolution mechanism.** Both paths execute **by name**;
they differ only in which directory the name resolves against:

| Source | Directory | Set `RUNTIME_CAPABILITIES_DIR`? |
|---|---|---|
| Binary's own seeded copies (smoke/demo workflows) | `<Home>/capabilities` | **No** — Home is the default |
| Company store (`capabilities-from-registry.yaml`, scaffold/ship demos) | the clone's `capabilities/` tree | **Yes** |
...
4. Smoke/demo workflows execute the capabilities **bootstrap seeded from
   the binary**, resolved **by name** against `<Home>/capabilities`
   (`runtime capability validate github/github-repo-health`). This needs
   **no** env var — Home is the default. Leave `RUNTIME_CAPABILITIES_DIR`
   unset in those workflows; it is reserved for the company store (rule 5,
   §F). This repo checks in no capability Markdown of its own.
...
| `runtime-home/` | Generated by bootstrap (gitignored) — includes `capabilities/`, the seeded copies workflows execute **by name** |

There is **no `examples/`**. It held a second copy of nine capabilities the
binary already ships; the copies drifted, so CI proved text no customer
had. Demo capabilities now come from the binary (via bootstrap) or the
company store — never from this repo's working tree.
...
5. **`runtime capability validate` before commit** for every capability a
   workflow you touched executes — bootstrap a home, then validate the
   seeded copy by name.
...
7. **Reusable library capabilities** belong in the company store
   ...
   a registry). This ci repo keeps **no** capability Markdown at all.
8. **Never re-vendor a capability here.** If a workflow needs one the
   binary does not ship, add it to `../engineering-runtime/capabilities/`
   (SSOT) or the company store, release, and resolve it by name. A local
   copy is how CI ends up proving text that no customer installs.
